//this component is pretty massive but it is the bread & butter of our application
// this component is how we can have a highl customizable interface
//with this component, depending on the props passed, we can change the size, color & type of vidual.
//this component also fetches data for itself and then passes it to its visual component.
...
class DashboardItem extends Component {
  constructor(props) {
    super(props);
    const { classes, size, metric, priority, visual, type } = this.props;
    this.state = {
      classNames: classes,
      size,
      metric,
        ...
        if (visual === 'chart') {
          ...
          try {
            value = strippedMetric.startsWith('ga:')
              ? data.data[strippedMetric]
              : data.data[`ga:${strippedMetric}`];
  ...
  getVisualComponent() {
    ...
    if (data === 'No data') {
      ...
      switch (visual) {
        case 'chart':
          component = (
            <ChartItem data={data} xKey="start" valKey="value" type={type} />
          );
          break;
        default:
          component = <TextItem data={data} />;
  ...
  render() {
    const { classNames, metric, size } = this.state;
